...
class LaporanController extends Controller
{
    ...
    public function laporanMingguan(Request $request)
    {
        ...
        // [FIX] Definisi variabel tanggalFormat untuk laporan mingguan
        $tanggalFormat  = Carbon::parse($dariTgl)->translatedFormat('d F Y') . ' - ' . Carbon::parse($sampaiTgl)->translatedFormat('d F Y');

        $pdf = Pdf::loadView('admin.laporan.pdf.mingguan', compact(
            'tabungan', 'penarikan', 'rekapJenis', 'rekapHarian',
            'totalKg', 'totalNilai', 'totalDicairkan', 'dariTgl', 'sampaiTgl', 'mingguKe', 'tanggalFormat'
        ))->setPaper('a4', 'portrait');
